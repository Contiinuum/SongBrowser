    ...
    internal static class Hooks
    {
        ...
        private static class Patch2SetMenuState
        {
            private static void Postfix(MenuState __instance, ref MenuState.State state)
            {
                ...
                if (!RandomSongButton.panelButtonsCreated)
                {
                    if (!RandomSongButton.buttonsBeingCreated && state == MenuState.State.SongPage) RandomSongButton.CreateSongPanelButton();
                    return;
                }
                if (state == MenuState.State.SongPage) MelonCoroutines.Start(RandomSongButton.SetRandomSongButtonActive(true));
                else if (state == MenuState.State.LaunchPage || state == MenuState.State.MainPage) MelonCoroutines.Start(RandomSongButton.SetRandomSongButtonActive(false));
